...
def regularized_nll_loss(args, model, output, target):
    ...
    if args.l2:
        for name, param in model.named_parameters():
            if name.split('.')[-1] in ["weight", "weight_orig"] and (name.split('.')[-2][:4] == "conv" or name.split('.')[-2][:2] == "fc"):
...
def apply_filter(model,device,args):
    ...
    for name, param in model.named_parameters():
        if name[:1] =="v":
            ...
            v_idx += 1
    for name, param in model.named_parameters():
        if len(param.shape)==4 and name.split('.')[-1] in ["weight", "weight_orig"]:
            if hasattr(model, "add_addm_v_layers"):
                param.data *= V[V_idx][:, None, None, None]
            else:
            ...
            V_idx += 1
        elif len(param.shape)==2 and name.split('.')[-1] in ["weight", "weight_orig"]:
